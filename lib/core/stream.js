/* jshint node:true */
'use strict';

const Transform = require('stream').Transform;

function Stream(parser){
  //super();
  Transform.apply(this, arguments)

  this._parser = parser;
  this._options = parser._options;
  this._headers = this._options.headers || [];
  this._hasHeaders = false;
  this._lastError = null;
  return this
}

Stream.prototype = Object.create(Transform && Transform.prototype, { constructor: { value: Stream, enumerable: false, writable: true, configurable: true } }); if (Transform) Object.setPrototypeOf ? Object.setPrototypeOf(Stream, Transform) : Stream.__proto__ = Transform;

Stream.prototype._mergeChunk = function(chunk) {
  let self = this;
  self._extra = self._extra || "";
  // Memory limit    
  if (self._extra.length > chunk.toString().length * 3) return null;
  // Remove starting comma
  if (self._extra.charAt(0) == ',') self._extra = self._extra.substr(1);
  // Append extra to chunk
  chunk = self._extra + chunk.toString();
  // Clear extra memory
  if (self._extra.length > 0) self._extra = "";
  return chunk;
}

Stream.prototype._wrapArray = function(data) {
  if (data.charAt(0) != '[') data = '[' + data;
  if (data.charAt(data.length - 1) != ']') data += ']';
  return data;
}

Stream.prototype._transform = function(chunk, encoding, done) {
  let self = this;
  let json = null;
  // Append extra data to chunk data
  chunk = this._mergeChunk(chunk);    
  if (!chunk) return done(this._lastError);
  // Split chunk in objects
  let parts = chunk.split('}');
  while (json === null && parts.length > 0) {
    try {
      let data = self._wrapArray(parts.join('}'));
      json = JSON.parse(data);
    } catch (ex) {
      this._lastError = ex;
      let extraChunk = parts.pop();
      self._extra = extraChunk + (self._extra || "");
      if (parts.length > 0) parts[parts.length - 1] += "}";
    }
  }    
  if (!json) return done();
  this._parser.parse(json, function(err, csvChunk){
    if (err) return done(err);
    if (!self.hasHeaders) {
      self.hasHeaders = true;
      self.push(self._parser.headers);
    }
    self.push(self._options.endOfLine + csvChunk);
    done();
  }, true);
}

module.exports = Stream;