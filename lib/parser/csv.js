...
class Parser {
  ...
  get headers() {
    ...
    if (this._options.rename && this._options.rename.length > 0)
      headers = headers.map(function(header){return this._options.rename[this._options.headers.indexOf(header)] || header}.bind(this))
  ...
  _checkRows(rows) {
    let lastRow = null;
    let finalRows = [];
    let fillGaps = function(col, index){return col === '' || col === undefined ? lastRow[index] : col}
    for (let row of rows) {
